...
index_ticker_list = [{"label" : "None", "value": ""}]
for value in compare_tickers:
    data_line = {"label" : str(value), "value": str(value)}
    index_ticker_list.append(data_line)
...
fund_ticker_list = [{"label" : "None", "value": ""}]
for value in df_sql['Name'].unique().tolist():
    data_line = {"label" : str(value), "value": str(value)}
    fund_ticker_list.append(data_line)
    ...
    [dbc.Col(
        dbc.FormGroup([
            ...
            options=[
                    ...
                    {"label": "Black color", "value": "Black color"}
...
inputrow3_7 = dbc.Row(
    [dbc.Col(
        dbc.FormGroup([
            dbc.Label('Single Mint fund'),
            dcc.Dropdown(
            id='checklistindexPLOT7',
            options=[
                    {"label": "None", "value": ""},
                    {"label": "Single Fund Plot", "value": "Single Fund Plot"},
                ...
                dbc.Row(
                    [dbc.Col(
                        [html.P('Through this dashboard you can create graphs for a specific range based on a Mint Tower fund and an index. Left date is the START, Right date is the END'),
                         ...
                         dbc.Button('Plot Graph single fund', id='button2', n_clicks=int(0))]),                        
...
@app.callback( 
    ...
    [State('bid','date'),
     ...
     State('checklistindexPLOT7','value'),
     ]
                )


#this function activates the code when clicking on "Plot graph" button in the dashboard
def update_sheet(clicks, bid, ask, checklistclassPLOT, checklistindexPLOT, checklistindexPLOT2, checklistindexPLOT3, checklistindexPLOT4, checklistindexPLOT5, checklistindexPLOT6, checklistindexPLOT7): #checklistindexPLOT4, 
    
    if clicks == 0:
        raise PreventUpdate
            
    else:
        if checklistindexPLOT != "":

            def create_variables():
                global Datebefore, Datenow, Fund_Name, Index_Name, Index_Name2, Index_Name3, Text, Legend, Axis_color, Title, single_fund
                Datebefore = bid
                Datenow = ask
                ...
                single_fund = checklistindexPLOT7
            create_variables()
            ...
            def create_figure():
                ...
                if Index_Name != "":
                    if single_fund == "Single Fund Plot":
                        fig.add_trace(
                                        go.Scatter(
                ...
                if Title == "ENG TITLE":                 
                    if Legend == "WITH LEGEND" and Axis_color == "Gold color":
                        fig.update_layout(title={'text': "Net Cumulative Return",'y':0.9,'x':0.5,'xanchor': 'center', 'yanchor':'top'},showlegend=True, legend_traceorder="reversed", font_size=15, font_color = 'RGB(184, 134, 11)', paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                        ...
                        fig.update_layout(title={'text': "Net Cumulative Return",'y':0.9,'x':0.5,'xanchor': 'center', 'yanchor':'top'},showlegend=True, legend_traceorder="reversed", font_size=15, font_color = "white", paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                                          xaxis =dict(showgrid = False, tickangle=-45), yaxis=dict(gridcolor = 'rgba(0, 0, 0, 0)'))
                    if Legend == "WITH LEGEND" and Axis_color == "Black color":
                        fig.update_layout(title={'text': "Net Cumulative Return",'y':0.9,'x':0.5,'xanchor': 'center', 'yanchor':'top'},showlegend=True, legend_traceorder="reversed", font_size=15, font_color = 'RGB(0,0,0)', paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                                          xaxis =dict(showgrid = False, tickangle=-45), yaxis=dict(gridcolor = 'RGB(0,0,0)'))                    
                    
                    elif Legend == "NO LEGEND" and Axis_color == "Gold color":
                        fig.update_layout(title={'text': "Net Cumulative Return",'y':0.9,'x':0.5,'xanchor': 'center', 'yanchor':'top'},showlegend=False, legend_traceorder="reversed", font_size=15, font_color = 'RGB(184, 134, 11)', paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                                          xaxis =dict(showgrid = False, tickangle=-45), yaxis=dict(gridcolor = 'RGB(184, 134, 11)'))
                    elif Legend == "NO LEGEND" and Axis_color == "White color":
                        fig.update_layout(title={'text': "Net Cumulative Return",'y':0.9,'x':0.5,'xanchor': 'center', 'yanchor':'top'},showlegend=False, legend_traceorder="reversed", font_size=15, font_color = "white", paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                                          xaxis =dict(showgrid = False, tickangle=-45), yaxis=dict(gridcolor = 'rgba(0, 0, 0, 0)'))      
                    elif Legend == "NO LEGEND" and Axis_color == "Black color":
                        fig.update_layout(title={'text': "Net Cumulative Return",'y':0.9,'x':0.5,'xanchor': 'center', 'yanchor':'top'},showlegend=False, legend_traceorder="reversed", font_size=15, font_color = 'RGB(0,0,0)', paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                                          xaxis =dict(showgrid = False, tickangle=-45), yaxis=dict(gridcolor = 'RGB(0,0,0)'))                      
                        
        
                if Title == "DUTCH TITLE":                 
                    if Legend == "WITH LEGEND" and Axis_color == "Gold color":
                        fig.update_layout(title={'text': "Netto Rendementsverloop",'y':0.9,'x':0.5,'xanchor': 'center', 'yanchor':'top'},showlegend=True, legend_traceorder="reversed", font_size=15, font_color = 'RGB(184, 134, 11)', paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                                          xaxis =dict(showgrid = False, tickangle=-45), yaxis=dict(gridcolor = 'RGB(184, 134, 11)'))
                    elif Legend == "WITH LEGEND" and Axis_color == "White color":
                        fig.update_layout(title={'text': "Netto Rendementsverloop",'y':0.9,'x':0.5,'xanchor': 'center', 'yanchor':'top'},showlegend=True, legend_traceorder="reversed", font_size=15, font_color = "white", paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                                          xaxis =dict(showgrid = False, tickangle=-45), yaxis=dict(gridcolor = 'rgba(0, 0, 0, 0)'))
                    elif Legend == "WITH LEGEND" and Axis_color == "Black color":
                        fig.update_layout(title={'text': "Netto Rendementsverloop",'y':0.9,'x':0.5,'xanchor': 'center', 'yanchor':'top'},showlegend=True, legend_traceorder="reversed", font_size=15, font_color = 'RGB(0,0,0)', paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                                          xaxis =dict(showgrid = False, tickangle=-45), yaxis=dict(gridcolor = 'RGB(0,0,0)'))
                        
                    elif Legend == "NO LEGEND" and Axis_color == "Gold color":
                        fig.update_layout(title={'text': "Netto Rendementsverloop",'y':0.9,'x':0.5,'xanchor': 'center', 'yanchor':'top'},showlegend=False, legend_traceorder="reversed", font_size=15, font_color = 'RGB(184, 134, 11)', paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                                          xaxis =dict(showgrid = False, tickangle=-45), yaxis=dict(gridcolor = 'RGB(184, 134, 11)'))
                    elif Legend == "NO LEGEND" and Axis_color == "White color":
                        fig.update_layout(title={'text': "Netto Rendementsverloop",'y':0.9,'x':0.5,'xanchor': 'center', 'yanchor':'top'},showlegend=False, legend_traceorder="reversed", font_size=15, font_color = "white", paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                                          xaxis =dict(showgrid = False, tickangle=-45), yaxis=dict(gridcolor = 'rgba(0, 0, 0, 0)'))  
                    elif Legend == "NO LEGEND" and Axis_color == "Black color":
                        fig.update_layout(title={'text': "Netto Rendementsverloop",'y':0.9,'x':0.5,'xanchor': 'center', 'yanchor':'top'},showlegend=False, font_size=15, font_color = 'RGB(0,0,0)', paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                                          xaxis =dict(showgrid = False, tickangle=-45), yaxis=dict(gridcolor = 'RGB(0,0,0)'))                    
                        
                if Title == "NO TITLE":                 
                    if Legend == "WITH LEGEND" and Axis_color == "Gold color":
                        fig.update_layout(showlegend=True, legend_traceorder="reversed", font_size=15, font_color = 'RGB(184, 134, 11)', paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                                          xaxis =dict(showgrid = False, tickangle=-45), yaxis=dict(gridcolor = 'RGB(184, 134, 11)'))
                    elif Legend == "WITH LEGEND" and Axis_color == "White color":
                        fig.update_layout(showlegend=True, legend_traceorder="reversed", font_size=15, font_color = "white", paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                                          xaxis =dict(showgrid = False, tickangle=-45), yaxis=dict(gridcolor = 'rgba(0, 0, 0, 0)'))
                    if Legend == "WITH LEGEND" and Axis_color == "Black color":
                        fig.update_layout(showlegend=True, legend_traceorder="reversed", font_size=15, font_color = 'RGB(0,0,0)', paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                                          xaxis =dict(showgrid = False, tickangle=-45), yaxis=dict(gridcolor = 'RGB(0,0,0)'))                
                    
                    elif Legend == "NO LEGEND" and Axis_color == "Gold color":
                        fig.update_layout(showlegend=False, legend_traceorder="reversed", font_size=15, font_color = 'RGB(184, 134, 11)', paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                                          xaxis =dict(showgrid = False, tickangle=-45), yaxis=dict(gridcolor = 'RGB(184, 134, 11)'))
                    elif Legend == "NO LEGEND" and Axis_color == "White color":
                        fig.update_layout(showlegend=False, legend_traceorder="reversed", font_size=15, font_color = "white", paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                                          xaxis =dict(showgrid = False, tickangle=-45), yaxis=dict(gridcolor = 'rgba(0, 0, 0, 0)'))  
                    elif Legend == "NO LEGEND" and Axis_color == "Black color":
                        fig.update_layout(showlegend=False, legend_traceorder="reversed", font_size=15, font_color = 'RGB(0,0,0)', paper_bgcolor = 'rgba(0, 0, 0, 0)', plot_bgcolor = 'rgba(0, 0, 0, 0)', grid_xgap =0, grid_ygap=0,
                                          xaxis =dict(showgrid = False, tickangle=-45), yaxis=dict(gridcolor = 'RGB(0,0,0)'))                    
